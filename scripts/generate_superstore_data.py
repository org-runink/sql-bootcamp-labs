#!/usr/bin/env python3
"""
Generates deterministic seed data for the `superstore` lab database.
...
CSV_DIR = Path(__file__).resolve().parent.parent / "exercises" / "data"
...
def main():
    ...
    write_csv(CSV_DIR / "returns.csv",
              ["OrderID", "Status"],
              [r for r in returns if r[0] <= 400])

    (OUT_DIR / "01_superstore_products.sql").write_text(
    ...
    (OUT_DIR / "02_superstore_customers.sql").write_text(
        "-- Auto-generated by scripts/generate_superstore_data.py\n" +
        sql_insert(
            "customers",
            ["CustomerID", "CustomerName", "Province", "Region", "CustomerSegment"],
    ...
    (OUT_DIR / "03_superstore_orders.sql").write_text(
        "-- Auto-generated by scripts/generate_superstore_data.py\n" +
        sql_insert(
            "orders",
            ["OrderID", "ProductID", "CustomerID", "OrderDate", "OrderPriority", "OrderQuantity",
    ...
    (OUT_DIR / "04_superstore_returns.sql").write_text(
        "-- Auto-generated by scripts/generate_superstore_data.py\n" +
        sql_insert("returns", ["OrderID", "Status"], returns)
    )
