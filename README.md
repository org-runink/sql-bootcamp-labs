...
├── week3_recap/              # RECAP, not a class: three sheets on the week's recurring traps
│   ├── README.md             #   grain/fan-out, rows that vanish, numbers you must not average
...
└── scripts/
    ...
    └── run_all_solutions.py          # EXECUTES every solution; diffs against a baseline
```
...
- **[`week3_recap/`](week3_recap/README.md)** — **not a class, a recap.** Three
  sheets, thirty questions, sized to drop into whichever sessions follow week 3.
  Everything in them already appeared in week 3, but on four different days
  against four different datasets; these put all of it on **one** dataset so the
  pattern is visible. Sheet 01 is grain and fan-out, 02 is rows that silently
...
## Checking the solutions still run

`check_exercises.py` reads the notebooks. **`run_all_solutions.py` runs them**,
in the lab image, and is the only check that catches a solution that stopped
working — a cell that now raises, a data file that moved, a pandas behaviour
that shifted under a version bump.

```bash
python3 scripts/run_all_solutions.py                   # ~15 min
python3 scripts/run_all_solutions.py --update-baseline
```

It compares against `scripts/solution_errors.json` rather than treating any
error as a failure, because **many solutions end in a deliberate error** — the
error *is* the lesson. `week2_day4_afternoon` documents eleven of them, and they
are not in the last cell. So a *count* means nothing and a *change* means
everything. When you deliberately add or remove a raising question, re-record the
baseline in the same commit.

It skips `wcd-originals/`: those vendor notebooks read their data from S3, so
their result depends on the network rather than on this repo.
...
```bash
podman ps        # a stale console shows (unhealthy)
```

and `check_console.py` reports it explicitly, with the rebuild command.
